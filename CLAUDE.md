...
### Playwright — только по явному запросу
**КРИТИЧЕСКИ ВАЖНО:** Использовать Playwright (браузерное тестирование) **ТОЛЬКО** при явном запросе пользователя. Не запускать Playwright самостоятельно для проверки UI-изменений.
...
### Обучение (тур) на странице после нового функционала
**КРИТИЧЕСКИ ВАЖНО:** После добавления или изменения функционала на странице **ОБЯЗАТЕЛЬНО** проверить, нужно ли обновить обучение (тур) этой страницы, и при необходимости обновить его в том же изменении.
- Шаги тура хранятся в `frontend/src/app/[feature]/_components/constants/[feature]-tour.constants.ts`, цели шагов — атрибуты `data-tour` в разметке
- Проверить: описан ли новый элемент/поведение, не устарели ли тексты шагов (правила расчёта, названия кнопок и колонок), не пропали ли элементы с `data-tour`, на которые ссылаются шаги
- Если функционал удалён или переименован — удалить или исправить соответствующие шаги
- В итоговом сообщении явно указать: тур обновлён или обновление не требуется
