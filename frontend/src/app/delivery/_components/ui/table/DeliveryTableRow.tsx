...
import DeliveryCommentsCell from './cells/DeliveryCommentsCell';
...
export interface DeliveryRowHandlers {
  ...
  /** Открывает попап комментариев поставки у ячейки */
  onOpenComments: (delivery: Delivery, anchor: DOMRect) => void;
  /** id поставки, у которой открыт попап комментариев */
...
export default function DeliveryTableRow({
  row, isFirst, onOpen, onChangePlannedDate, onChangeActualDate, onChangeEsfDate, onOpenComments, openCommentsId, backUrl, onNavigate,
}: Props) {
  ...
  return (
    <div
      ...
      onClick={() => onOpen(d)}
      className={`${DELIVERY_GRID_CLASS} items-start px-5 py-[9px] border-b border-slate-100 text-[12px] leading-[1.35] cursor-pointer bg-white hover:bg-slate-50 ${stripe}`}
    >
      ...
      <div className="pr-2 min-w-0 text-slate-900 line-clamp-2" title={d.contractSubject ?? undefined}>{d.contractSubject || '—'}</div>
      ...
      <div className="pr-2 min-w-0">
        <DeliveryCommentsCell
          count={d.commentsCount}
          isOpen={openCommentsId === d.id}
          onOpen={(anchor) => onOpenComments(d, anchor)}
        />
      </div>
      <div
