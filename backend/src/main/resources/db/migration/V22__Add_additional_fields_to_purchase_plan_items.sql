-- Add additional fields to purchase_plan_items table (with existence checks)
DO $$
BEGIN
    ...
    IF NOT EXISTS (SELECT 1 FROM information_schema.columns WHERE table_name = 'purchase_plan_items' AND column_name = 'auto_renewal') THEN
        ALTER TABLE purchase_plan_items ADD COLUMN auto_renewal BOOLEAN;
    END IF;
    
    IF NOT EXISTS (SELECT 1 FROM information_schema.columns WHERE table_name = 'purchase_plan_items' AND column_name = 'complexity') THEN
        ALTER TABLE purchase_plan_items ADD COLUMN complexity VARCHAR(255);
    END IF;
