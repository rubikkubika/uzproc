...
public class PurchasePlanVersionController {
    ...
    @GetMapping("/{id}/items")
    public ResponseEntity<List<com.uzproc.backend.dto.purchaseplan.PurchasePlanItemDto>> getVersionItems(@PathVariable Long id) {
        try {
            List<com.uzproc.backend.dto.purchaseplan.PurchasePlanItemDto> items = versionService.getVersionItems(id);
