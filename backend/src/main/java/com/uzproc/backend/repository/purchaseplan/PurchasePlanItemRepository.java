...
public interface PurchasePlanItemRepository extends JpaRepository<PurchasePlanItem, Long>, JpaSpecificationExecutor<PurchasePlanItem> {
    ...
    // Поиск всех уникальных годов (раздельно для действующего плана и драфта; позиции, скрытые очисткой драфта, не учитываются)
    @org.springframework.data.jpa.repository.Query("SELECT DISTINCT p.year FROM PurchasePlanItem p WHERE p.year IS NOT NULL AND p.isDraft = :isDraft AND (p.isDraft = false OR p.draftCleared = false) ORDER BY p.year DESC")
    ...
    // Позиции драфта за год, включая скрытые очисткой (для генерации из договоров без дублей и с сохранением id)
